...
function verificationWarnings(routerOutput, root) {
  ...
  if (/verify-harness/.test(cmd) && /&&/.test(cmd)) {
    warnings.push(
      `verification command combines verify-harness with another command through &&. ` +
      `verify-harness checks the whole repository state (including this feature), ` +
      `which can create a circular dependency. Consider separating it into a command that checks only this feature's behavior.`
    );
  }
  return warnings;
}
