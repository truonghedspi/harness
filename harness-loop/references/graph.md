...
| `loop/route-log.jsonl` | `run-loop.sh` (the dispatcher) | `route.mjs` | append-only. What was actually dispatched, per marker. The router reads it to tell "this node has not had a turn on this marker" from "it had one and nothing changed" — the router stays pure, the dispatcher records |
