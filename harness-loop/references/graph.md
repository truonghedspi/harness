...
| `orchestrator` | agent | **the front door** — reports state, natively spawns one sub-agent with the exact role `route.mjs` names, and escalates decisions. After a newly observed `done`, it reads the three telemetry reports, proposes at most one evidence-backed improvement, and routes it only after explicit human approval. Script dispatch is fallback only. The default role when a human names none. **Never chooses the node**, and writes no product file | `AGENTS.md`, `graph.md`, `human-attention.md`, `presenting-and-proposing.md`, `loop-status.mjs`, `route.mjs`, telemetry reports | `session-handoff.md`, `progress.md`, `loop/approval.md`, `memory/orchestrator/**` |
...
or rejection in `session-handoff.md`; approval routes a project change to `feature-planner` and a
canonical harness change through the issue/improvement pipeline. The orchestrator cannot turn a
telemetry signal into a source edit by itself.
