...
| `tools/guard-write.mjs` | code | classify an edit against an agent's `writes`, then adapt output per runtime: Claude emits allow/deny; Codex emits neutral/deny | `agents.manifest.json`, runtime, tool payload | runtime-specific PreToolUse response |
