...
`node tools/work-split.mjs validate <feat-id>`, read by `loop/route.mjs` and `tools/guard-write.mjs`.
...
```json
{
  "kind": "work-split/1",
  "feature": "feat-x",
  "contractDigest": "<node tools/review-contract.mjs feat-x --json>",
  ...
  "slices": [
    {
      ...
      "mustRead": ["docs/design/order-flow.md#seam", "src/api/types.ts"],
