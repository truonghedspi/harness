...
- MUST fix reusable behavior in `harness-loop/templates/**` or `harness-loop/scripts/**`; a root
  dogfood copy is evidence and a consumer, not the canonical implementation.
- MUST add a `demo.sh` assertion for every harness behavior change.
...
- MUST NOT use `--force` to make this existing repository match a fresh scaffold.
...
| Canonical source and regression proof | review plus `bash harness-loop/scripts/demo.sh` |
