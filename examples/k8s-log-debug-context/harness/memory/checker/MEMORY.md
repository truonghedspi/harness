...
- [Attribute mapped from unemitted field](attribute-mapped-from-unemitted-field.md) — A green config test can map a v1 field from a k8s attribute the pinned upstream never emits (k8s.workload.name); the mapping is present so a static test passes, but resolves blank at runtime.
