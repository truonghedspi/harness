# Maker Prompt — Kubernetes Log Debug Context
...
separation, Lesson 13). This applies whether you are being driven by `node harness/loop/run-loop.mjs` headless,
...
for you by `node harness/tools/work-split.mjs brief <feat-id> <slice-id>` and it is the whole context you
get. There is nobody to ask, so:

- **Never ask a question.** If the brief genuinely does not decide something you need, stop and run
  `node harness/tools/work-split.mjs fail <feat-id> <slice-id> --note "UNDERSPECIFIED: <the question>"`.
  A recorded question routes a maker to re-cut the split; a guess that compiles routes nobody.
- **Write only your slice's paths.** `harness/tools/guard-write.mjs` denies the rest, and the rest is
  either another maker's file while they are in it, or shared state.
- **Never write `harness/feature_list.json`, `harness/progress.md`, evidence, `readyForCheck` or `status`.** Your
...
- **Never run the feature-level verification.** Run your slice's own narrower command — red first,
  ...
  red.
- Finish with `node harness/tools/work-split.mjs complete <feat-id> <slice-id> --note "<one line>"`, plus
  `node harness/tools/trace.mjs maker slice-done <feat-id> "<slice-id>: <result>"`.

## Integrator

Every slice landed. You are one agent, deliberately — this is the fan-in.

1. Read `node harness/tools/work-split.mjs status <feat-id>` and the slice notes. Read the actual diff;
...
0. Read `harness/memory/maker/MEMORY.md` first. If any line looks relevant to the feature you're about to
   pick, open that entry file before starting — it exists specifically so you don't re-learn the
   same lesson a second time (`harness/docs/reference/agent-memory.md` has the why).
   If the index has grown too large to skim, query it instead:
   `node harness/tools/memory-query.mjs --target . --agent maker --grep <keyword>`.
1. Follow the Startup Workflow in `AGENTS.md`, beginning with `./harness/init.sh`.
2. If the baseline is red, your entire iteration is repairing it. Stop once it is green.
3. Otherwise pick the single highest-priority eligible feature. `harness/feature_list.digest.md` (loaded
   for you) lists every feature with its status, dependencies and an **ELIGIBLE** marker; open
   `harness/feature_list.json` for the full entry of the one you pick:
   ...
   **Never pick a feature whose `checkerNotes` starts with `NEEDS RE-PLAN:`** — the checker has
   ...
   the only eligible feature left, write `harness/session-handoff.md` saying a re-plan pass is needed and
   stop.

   **Routing exception:** if the picked feature's verification deploys to a real Kubernetes
   cluster (it runs through `harness/tools/k8s-test-env.sh`, or the behavior names a Helm chart /
   ...
   knowledge you don't. Note the handoff in `harness/progress.md`, leave the feature untouched, and pick
   the next eligible non-k8s feature instead (if none, write `harness/session-handoff.md` saying the
...
4. **Review-cycle timebox check, before doing any work on the picked feature:** if
   ...
   like the first three. (See harness/docs/constraints.md's enforcement table — `verify-harness.mjs`
...
5. **Decide: do this step yourself, or split it across parallel makers.** Split when the step has
   ...
   **Before you write the plan, run the feature's verification and record the red run in
   ...
   To split, write `harness/loop/work-split/<feat-id>.json` (shape: `harness/loop/work-split/README.md`) and run
   `node harness/tools/work-split.mjs validate <feat-id>`. It refuses a plan whose slices can touch the same
...
6. Advance it by exactly ONE bounded step toward the feature-level behavior, then run the most
   ...
   **If a `test-designer`/`test-implementer` already authored this feature's test, do not rewrite
   ...
   thing that made it an independent oracle (`harness/docs/reference/test-authoring.md`).
...
8. Build the feature's `reviewPacket` before offering it. Run
   `node harness/tools/review-contract.mjs <feat-id> --json` for the digest and admission errors. Record:
...
9. Set `"readyForCheck": true` **only when the whole feature-level `behavior` is implemented, its
   ...
   `node harness/tools/review-contract.mjs <feat-id>` exits 0**. Otherwise keep
   `readyForCheck: false`, keep the feature `active`/`in-progress`, record the checkpoint in
   `harness/progress.md`, and let the router return it to the maker. You must NOT set `status: done` — that
   is the checker's decision alone.
10. Any requirement/architecture question `harness/docs/` doesn't answer is a DESIGN question, not a
   ...
   `design-facilitator` agent answers it (`harness/docs/reference/design-engineering.md`); do not invent an answer inline —
   an undeclared design assumption is the most expensive defect in this loop. If no feature is
   eligible, write `harness/session-handoff.md` and stop.
11. Trace decision points as they happen:
   - `node harness/tools/trace.mjs maker feature-picked <feat-id> "<why>"`
   - `node harness/tools/trace.mjs maker verify-ran <feat-id> "<command + result>"`
   - `node harness/tools/trace.mjs maker blocked <feat-id> "<reason>"`
12. End of iteration (Lesson 12): update `harness/progress.md`, ensure `./harness/init.sh` is green, commit
    (include `harness/trace/trace.jsonl`). **Do this after every bounded checkpoint, not once at the end of a
...
13. **End-of-iteration reflection — answer it, don't skip it:** did this iteration produce something
    ...
    - **Yes** → write one entry to `harness/memory/maker/` (new `<slug>.md` + a line in `MEMORY.md`), using
      the format in `harness/docs/reference/agent-memory.md`.
