...
async function main() {
  ...
  for (let iteration = 1; iteration <= iterations; iteration++) {
    ...
    if (next.kind === "agent") {
      ...
      const headless = `You are running HEADLESS under node harness/loop/run-loop.mjs — no human can answer questions, so commit directly instead of asking. The router selected you because: ${next.why}. Run exactly one iteration per your instructions and loop/goal.md. Honor every stop condition.`;
