# Definition of Done — Kubernetes Log Debug Context
...
1. **Behavior implemented** — the feature's stated `behavior` in `harness/feature_list.json` works.
2. **Verification ran and passed** — the feature's `verification` command was actually executed,
   at the correct level (`harness/docs/testing-standards.md`). Not "should pass" — *did* pass.
3. **Evidence recorded** — command, one-line summary, and date written into the feature's
   `evidence` field.
4. **Clean and restartable** — `./harness/init.sh` is green from a clean checkout; no stale artifacts.
...
2. Unwritten conventions → add to `harness/docs/constraints.md`.
3. Incomplete environment → fix `init.sh`.
4. No verification → add a real `verification` command.
5. Lost cross-session state → update `harness/progress.md` / `harness/DECISIONS.md`.

Then re-run. Keep a short failure note in `harness/progress.md` so the same layer isn't rediscovered next
session.
