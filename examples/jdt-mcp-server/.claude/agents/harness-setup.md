...
hooks:
  ...
  PostToolUse:
    - matcher: "Read|Grep|Glob|Bash"
      command: "node harness/tools/telemetry.mjs --runtime claude --actor harness-setup"
---

<!-- GENERATED from agents.manifest.json + harness/prompts/harness-setup.md by tools/gen-agents.mjs. Do not hand-edit:
     your change is lost on the next generation, and the two runtimes silently diverge. -->

> **What this role does:** Harness setup: I will verify the toolchain, confirm MCP connectivity, fill in the real verification commands, and get ./harness/init.sh green. Setup only — no feature work.

# Harness Setup — JDT MCP Server
...
`harness/memory/harness-setup/` (new `<slug>.md` + a line in `MEMORY.md`) before you finish. Don't write
