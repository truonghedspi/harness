...
3. **Spawn one sub-agent** with the exact role `route.mjs` named. Use the runtime's native
   ...
   native sub-agent facility, fall back to `node harness/loop/run-loop.mjs 1`; that adapter exists for
...
Write what you learned into `harness/memory/orchestrator/` and leave `harness/session-handoff.md` current, so the
