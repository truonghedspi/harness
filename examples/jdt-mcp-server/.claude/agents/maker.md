...
7. Set `"readyForCheck": true`. You must NOT set `"status": "done"` — that is the checker's
   decision alone.
8. Any requirement/architecture question `harness/docs/` doesn't answer is a DESIGN question, not a
...
10. End of iteration (Lesson 12): update `harness/progress.md`, ensure `./harness/init.sh` is green, commit
    (include `harness/trace/trace.jsonl`). **Do this after every single feature, not once at the end of a
...
11. If this iteration taught you something the *next* maker run on this project shouldn't have to
    ...
    to `harness/memory/maker/` (new `<slug>.md` + a line in `MEMORY.md`), using the format in
    `harness/docs/reference/agent-memory.md`. Don't write one for a routine, expected iteration — that's noise,
    not a lesson.
