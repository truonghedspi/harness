...
- **Active feature:** none — feat-prove-provisioner timebox-blocked at 3/3
- **Latest commit:** pending — maker enforced the attempt ceiling without another feature verification run
- **Baseline (`./harness/init.sh`):** green at iteration end — six baseline integration cases and four lsp-client unit cases passed
...
The prove-provisioner feature is rejected: its 13-case green replay lacks corrupt-download/checksum-rejection coverage. The lsp-client still needs a cross-process oracle; project-router's oracle gap is closed and awaits implementation.
