...
- **Active feature:** feat-lsp-client — attempt 2/4, ready for checker. Added `test/integration/lsp-client-process.integration.spec.ts`, a Level 3 oracle that spawns a real Node child speaking Content-Length-framed stdio, proves framing/correlation over that real boundary, then SIGKILLs the child with two requests genuinely in flight and asserts both reject instead of hanging (INV-POOL-3). Confirmed the oracle is not vacuous by temporarily stripping the exit-handler's pending-rejection loop (red), then reverting (green, `src/lsp/lsp-client.ts` diff empty). Feature `verification` now runs the unit suite and this integration file together.
- **Latest commit:** pending — this iteration's work not yet committed
...
- [ ] feat-lsp-client — ready for checker after attempt 2/4
  - Added the required cross-process integration oracle (`test/integration/lsp-client-process.integration.spec.ts`): real spawned Node child, real Content-Length stdio, two requests in flight, real SIGKILL, both reject with `/JDT LS process exited/`. Proved the oracle can fail (temporarily broke the exit handler, reran red) before restoring it and rerunning green. Unit suite (`test/lsp/lsp-client.spec.ts`) unchanged, still 4/4.
...
2. Checker to replay `feat-lsp-client`'s new cross-process oracle (`test/integration/lsp-client-process.integration.spec.ts`) and decide.
