...
| A-021 | Two live workspaces can resolve to the same canonical file URI (a symlink alias, a nested reactor root, or a shared source directory), so diagnostics-cache identity is `(workspaceId, URI)` and `workspaceId` is a load-bearing key component, never redundant | assumed | `INV-DIAG-3` can only be violated at the unit level and the integration condition overstates its reach | — | `diagnostics-cache`, `INV-DIAG-3` |
