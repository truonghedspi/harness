...
| `INV-ROUTE-4` | `project-router` | A Gradle path always derives its project root from its own path: use the outermost enclosing ancestor `settings.gradle`/`settings.gradle.kts` (the multi-project root, analogous to the Maven reactor); if none encloses the path, use the nearest ancestor `build.gradle`/`build.gradle.kts` (analogous to the nearest `pom.xml`). Never a sticky, cached or default instance | `resolveWorkspace(path)` over a Gradle fixture tree (single-module and multi-project) |

`INV-ROUTE-1`/`INV-ROUTE-2`/`INV-ROUTE-3` stay Maven-specific — their oracles (`TCON-ROUTE-0001..0005`) are already `done` and rewordinging them would silently change what those contracts mean. `INV-ROUTE-4` is **additive**: it extends the router to Gradle with the same walk-up-and-pick pattern. Precedence in a mixed tree (a path under both a `pom.xml` and a Gradle marker — rare) is, outermost-first: `settings.gradle` root > Maven reactor `pom.xml` > nearest `build.gradle` > nearest `pom.xml`. A path with no `pom.xml` **and** no Gradle marker still yields the explicit named-path error of `INV-ROUTE-2` (its wording says "pom.xml" today; the error-message text may generalise to "no Maven or Gradle workspace" without changing the invariant's claim — it only requires an explicit error naming the path, never an empty success).
