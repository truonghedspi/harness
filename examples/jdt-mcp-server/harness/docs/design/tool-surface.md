...
| `INV-DIAG-4` | `daemon` | A file-scoped `java_diagnostics` answer is produced only after the daemon has waited, bounded by the call deadline, for the queried document's `publishDiagnostics` — "not reported yet" then means only "validation still in flight at the deadline", never "the daemon read the cache before JDT LS had a chance to publish" | call `java_diagnostics` for a file on an already-imported workspace (no fresh m2e import); assert `reported`, not `not-reported` |
...
`INV-DIAG-4` exists because `publishDiagnostics` is a push that lands **after** a workspace is
(re)acquired, not synchronously with it. The original `java_diagnostics` read the cache immediately
and could answer `not-reported` before that publish arrived — so an already-imported workspace looked
permanently uncomputed. The daemon (composition root, `src/cli.ts`) therefore waits, bounded by the
call deadline, for the queried document's publish, and sends `textDocument/didOpen` first so JDT LS
validates the document immediately instead of on the re-import's own schedule. The wait is the
load-bearing half; the didOpen is a trigger that makes the publish arrive sooner. Both live on the
wiring layer, not on `diagnostics.ts`, which keeps its "never sends an LSP request" invariant.
