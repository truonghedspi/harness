...
//
// Usage:
//   node harness/loop/route.mjs                 # print the next node + why (human-readable)
//   node harness/loop/route.mjs --json          # same, machine-readable
//   node harness/loop/route.mjs --agent         # print just the agent name, or nothing when it is code
//   node harness/loop/route.mjs --rules         # the whole routing table, in precedence order
...
const featureConditionsComplete = (feature) => {
  ...
  const available = conditionCitations();
...
if (args.includes("--rules")) {
  ...
  L.push("  What the router would pick RIGHT NOW: node harness/loop/route.mjs");
  L.push("");
  writeSync(1, L.join("\n") + "\n");
  process.exit(0);
}
