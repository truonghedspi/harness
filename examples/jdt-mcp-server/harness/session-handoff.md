...
- Branch / commit: current branch / HEAD `93f27f7`
- Revalidated: 2026-08-20 iteration 15; both dependencies remain `not-started`, no callable `src/` interface exists, and `./harness/init.sh` is green.
...
The router has dispatched this proof with the same dependency blocker for fourteen consecutive iterations. Treat that ordering as a harness-routing defect; rerunning the same test-implementer iteration cannot create behavioral red evidence.
