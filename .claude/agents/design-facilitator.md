...
hooks:
  ...
  PreToolUse:
    - matcher: "Edit|Write|NotebookEdit|Bash"
      command: "node tools/guard-write.mjs design-facilitator"
  SubagentStop:
    - command: "node tools/trace.mjs design-facilitator session-end"
  PostToolUse:
    - matcher: "Read|Grep|Glob|Bash"
      command: "node tools/telemetry.mjs --runtime claude --actor design-facilitator"
---

<!-- GENERATED from agents.manifest.json + prompts/design-facilitator.md by tools/gen-agents.mjs. Do not hand-edit:
...
3. **State the strongest argument against your own recommendation** — the same discipline
   `prompts/orchestrator.md`'s one-way-door shape requires of every consequential ask, and a direct
...
- **End-of-session reflection — answer it, don't skip it:** did this session teach something
  ...
  **Yes** → write one entry to `memory/design-facilitator/` (`docs/reference/agent-memory.md` for
